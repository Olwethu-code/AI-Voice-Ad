...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen pb-20 relative overflow-x-hidden">
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 h-full w-64 bg-card border-r border-border z-[70] transition-transform duration-300 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:h-auto lg:z-0`}>
        <div className="p-6 space-y-8">
          ...
          <nav className="space-y-2">
            <button 
              onClick={() => { setInputType("text"); scrollTo(textInputRef); }}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-secondary transition-colors text-sm font-medium"
            >
              <Type className="w-4 h-4 text-primary" /> Promotional text
            </button>
            <button 
              onClick={() => { setInputType("image"); scrollTo(imageUploadRef); }}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-secondary transition-colors text-sm font-medium"
            >
              <ImageIcon className="w-4 h-4 text-accent" /> Upload image
            </button>
            <button 
              onClick={() => scrollTo(historyRef)}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-secondary transition-colors text-sm font-medium"
            >
              <History className="w-4 h-4 text-primary" /> Storing History / Ad Library
            </button>
          </nav>
        </div>
      </aside>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-5 space-y-8">
              <div ref={inputType === "text" ? textInputRef : imageUploadRef}>
              ...
              <form onSubmit={handleGenerate} className="glass-card rounded-3xl p-6 sm:p-8 space-y-8">
                ...
                <div className="bg-black/30 p-1.5 rounded-xl flex gap-1 border border-white/5">
                  ...
                  <button
                    ...
                    className={`flex-1 flex-center justify-center gap-2 py-3 rounded-lg text-sm font-semibold transition-all ${
                      inputType === "image" 
                        ? "bg-secondary text-foreground shadow-sm border border-white/10" 
                        : "text-muted-foreground hover:text-foreground hover:bg-white/5"
                    }`}
